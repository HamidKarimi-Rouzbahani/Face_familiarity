...
for region=[2]
    for stim_resp=[2]
        ...
        firingRatesPooled=nan*ones(1,2,2,2000,240);
        firingRatesAveragePooled=nan*ones(1,2,2,2000);
        trialNumPooled=zeros(1,2,2);
        
        for subj=[1:16 20 21]
            if region==3
                firingRates=nan*ones(62,2,2,2000,240);
                trialNum=zeros(62,2,2);
            else
                firingRates=nan*ones(14,2,2,2000,240);
                trialNum=zeros(14,2,2);
            end
...
%         toc
%         [region stim_resp]
%         save(['DPCA_FamUnf_region_',num2str(region),'_stim_resp_',num2str(stim_resp),'.mat'],'W','V','whichMarg');
        
        % plotting
        load(['DPCA_FamUnf_region_',num2str(region),'_stim_resp_',num2str(stim_resp),'.mat'],'W','V','whichMarg')
