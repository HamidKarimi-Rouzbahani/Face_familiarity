    ...
    for stim_resp=1:2
        ...
        for subj=[1:16 20 21]
            ...
            for time=span
                ...
                for coh=1:4
                    if stim_resp==1
                        load(['st_Decoding_fam_unfam_tr_all_ts_all_baselined_windowed_region_',num2str(region),'_Coh',num2str(noises(coh)),'V2.mat']);
                    else
                        load(['rp_Decoding_fam_unfam_tr_all_ts_all_baselined_windowed_region_',num2str(region),'_Coh',num2str(noises(coh)),'V2.mat']);
                    end
                    
                    decoding_accuracies(subj,(coh-1)*4+1:coh*4,t)=[nanmean(accuracy(subj,[1 2 3],time),2) nanmean(accuracy(subj,[1 4 5],time),2) nanmean(accuracy(subj,[2 4 6],time),2) nanmean(accuracy(subj,[3 5 6],time),2)];                   
        ...
        if stim_resp==1
            save(['st_aligned_Correlation_Betw_Decoding_Behaviour_region_',num2str(region),'.mat'],'corr_Dec_Beh_acc','corr_Dec_Beh_acc_upper','corr_Dec_Beh_acc_lower','significant');
        else
            save(['rp_aligned_Correlation_Betw_Decoding_Behaviour_region_',num2str(region),'.mat'],'corr_Dec_Beh_acc','corr_Dec_Beh_acc_upper','corr_Dec_Beh_acc_lower','significant');
        end
    end
end

%% 
plot(nanmean(corr_Dec_Beh_acc))
hold on;
plot(nanmean(corr_Dec_Beh_acc_upper))
plot(nanmean(corr_Dec_Beh_acc_lower))
