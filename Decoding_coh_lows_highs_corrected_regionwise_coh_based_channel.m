...
only_correct_trials2=1; % plus incorrect=0 you should also change the saving file names
            ...
            for subj=[1:16 20 21]
                ...
                for coherence=[1 2 3 4]
                    ...
                    for session=sessions
                        colors=[0 0.45 0.7 1];
                        indx{cc,session}=([Exp_data{1, session}.stim.stimTrain.imageNoise]==coherences(coherence));
                ...
                if test_on_errors==1
                    ...
                    signals2{1,3}=[];
                    signals2{1,4}=[];
                    clearvars -except coherence coherences region test_on_errors sizes_signals accuracy subject sessions steps baseline_span baseline_correction signals signals2 subj cats stim_resp only_answered_trials only_correct_trials only_incorrect_trials only_answered_trials2 only_correct_trials2 only_incorrect_trials2
                    %% SVM
                    cond1=1;
                    cond2=2;
                    time=0;
                    for tws=1:steps:2000-steps
                        ...
                        for iter=1:10
                            ...
                            %                         if bigger_class_ts==2
                            %                             ts_inds_bigger_class=randsample([1:size(signals2{1,cond2},3)],size(signals2{1,cond1},3));
                            %                             X2=[squeeze(mean(signals2{1,cond1}(:,tw,:),2))';squeeze(mean(signals2{1,cond2}(:,tw,ts_inds_bigger_class),2))'];
                            %                             y2=[ones(size(squeeze(signals2{1,cond1}(:,tws,:))',1),1);zeros(size(squeeze(signals2{1,cond1}(:,tws,:))',1),1)];
                            %                         else
                            %                             ts_inds_bigger_class=randsample([1:size(signals2{1,cond1},3)],size(signals2{1,cond2},3));
                            %                             X2=[squeeze(mean(signals2{1,cond1}(:,tw,ts_inds_bigger_class),2))';squeeze(mean(signals2{1,cond2}(:,tw,:),2))'];
                            %                             y2=[ones(size(squeeze(signals2{1,cond2}(:,tws,:))',1),1);zeros(size(squeeze(signals2{1,cond2}(:,tws,:))',1),1)];
                            %                         end
                            ...
                            %                         inds_tst1=ones(1,length(label));
                            %                         inds_tst2=logical([ones(1,length(ts_inds_bigger_class)) zeros(1,length(ts_inds_bigger_class))]);
                            %                         inds_tst3=logical([zeros(1,length(ts_inds_bigger_class)) ones(1,length(ts_inds_bigger_class))]);
                            %                         inds_tst4=logical([zeros(1,length(ts_inds_bigger_class)) (ts_inds_bigger_class<sizes_signals(2)+1)]);
                            %                         inds_tst5=logical([zeros(1,length(ts_inds_bigger_class)) (ts_inds_bigger_class>sizes_signals(2) & ts_inds_bigger_class<sizes_signals(2)+sizes_signals(3)+1)]);
                            %                         inds_tst6=logical([zeros(1,length(ts_inds_bigger_class)) (ts_inds_bigger_class>sizes_signals(2)+sizes_signals(3)+1 & ts_inds_bigger_class<sizes_signals(2)+sizes_signals(3)+sizes_signals(4)+1)]);
                    ...
                    cond2=2;
                    time=0;
                    [~,bigger_class]=max([size(signals{1,cond1},3) size(signals{1,cond2},3)]);
                    for tws=1:steps:2000-steps
                        tw=tws:tws+steps;
                        time=time+1;
%                         tic
                        for iter=1:10                           
                            if bigger_class==2
                                indices_tr1=randsample([1:size(signals{1,cond1},3)],fix(size(signals{1,cond1},3)*0.8));
                                indices_tr2=randsample([1:size(signals{1,cond2},3)],fix(size(signals{1,cond1},3)*0.8));
                            else
                                indices_tr1=randsample([1:size(signals{1,cond1},3)],fix(size(signals{1,cond2},3)*0.8));
                                indices_tr2=randsample([1:size(signals{1,cond2},3)],fix(size(signals{1,cond2},3)*0.8));
                            end
                            indices_ts1=[1:size(signals{1,cond1},3)];
                            indices_ts1(indices_tr1)=[];
                            indices_ts2=[1:size(signals{1,cond2},3)];
                            indices_ts2(indices_tr2)=[];
                            X1=[squeeze(mean(signals{1,cond1}(:,tw,indices_tr1),2))';squeeze(mean(signals{1,cond2}(:,tw,indices_tr2),2))'];
                            y1=[ones(length(indices_tr1),1);zeros(length(indices_tr2),1)];
                            X2=[squeeze(mean(signals{1,cond1}(:,tw,indices_ts1),2))';squeeze(mean(signals{1,cond2}(:,tw,indices_ts2),2))'];
                            y2=[ones(length(indices_ts1),1);zeros(length(indices_ts2),1)];
                            for channel=1:size(X1,2)
                                SVMModel = fitcsvm(X1(:,channel),y1);
                                [label,~] = predict(SVMModel,X2(:,channel));
                                
                                test_set_indices=1:length(label);
                                inds_tst1=logical(ones(1,length(label)));
                                inds_tst2=(test_set_indices<size(indices_ts1,2)+1);
                                inds_tst3=(test_set_indices>size(indices_ts1,2));
                                
                                inds_tst4=[logical(zeros(1,size(indices_ts1,2))) indices_ts2<sizes_signals(2)+1];
                                inds_tst5=[logical(zeros(1,size(indices_ts1,2))) (indices_ts2>sizes_signals(2) & indices_ts2<sizes_signals(2)+sizes_signals(3)+1)];
                                inds_tst6=[logical(zeros(1,size(indices_ts1,2))) (indices_ts2>sizes_signals(2)+sizes_signals(3) & indices_ts2<sizes_signals(2)+sizes_signals(3)+sizes_signals(4)+1)];
                                
                                % Unfamiliar vs Familiar
                                tmp(1,iter,channel)=sum(label(inds_tst1)==y2(inds_tst1))./sum(inds_tst1);
                                
                                % Unfamiliar and Familiar separately
                                tmp(2,iter,channel)=sum(label(inds_tst2)==y2(inds_tst2))./sum(inds_tst2);
                                tmp(3,iter,channel)=sum(label(inds_tst3)==y2(inds_tst3))./sum(inds_tst3);
                                
                                % Familiar categories in order: Famous, Familiar, Self
                                tmp(4,iter,channel)=sum(label(inds_tst4)==y2(inds_tst4))./sum(inds_tst4);
                                tmp(5,iter,channel)=sum(label(inds_tst5)==y2(inds_tst5))./sum(inds_tst5);
                                tmp(6,iter,channel)=sum(label(inds_tst6)==y2(inds_tst6))./sum(inds_tst6);
                            end
                        end
%                         toc
                        [test_on_errors region stim_resp coherence subj time]
                        accuracy(subj,1:6,time,1:62)=squeeze(nanmean(tmp,2));
                    end
                end
            end
            %             if stim_resp==1
            %                 save(['st_Decoding_fam_unfam_tr_cor_ts_cor_baselined_windowed_region_',num2str(region),'_Coh',num2str(coherences(coherence)),'channels.mat'],'accuracy');
            %             else
            %                 save(['rp_Decoding_fam_unfam_tr_cor_ts_cor_baselined_windowed_region_',num2str(region),'_Coh',num2str(coherences(coherence)),'channels.mat'],'accuracy');
            %             end
            if stim_resp==1
                save(['st_Decoding_coh_highs_lows_tr_cor_ts_cor_baselined_windowed_region_',num2str(region),'_channels.mat'],'accuracy');
            else
                save(['rp_Decoding_coh_highs_lows_tr_cor_ts_cor_baselined_windowed_region_',num2str(region),'_channels.mat'],'accuracy');
            end
            %         end
    end
end
