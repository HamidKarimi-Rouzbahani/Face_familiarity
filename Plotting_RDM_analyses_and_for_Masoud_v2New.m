...
%     hold on;
    ...
    for time=1:71
        if sum(time==inclusion_time)>0 && nanmean(True_correlations_Fam_Unfam_NP(:,time),1)>0 && sum(nanmean(True_correlations_Fam_Unfam_NP(:,time),1)>nanmean(Random_correlations_Fam_Unfam_NP(:,time,:),1))>((1-p_value).*10000)
            Significances_Fam_Unfam_NP(coherence,time)=1;
        else
            Significances_Fam_Unfam_NP(coherence,time)=nan;
        end
        if sum(time==inclusion_time)>0 && nanmean(True_correlations_Fam_Levels_NP(:,time),1)>0 && sum(nanmean(True_correlations_Fam_Levels_NP(:,time),1)>nanmean(Random_correlations_Fam_Levels_NP(:,time,:),1))>((1-p_value).*10000)
...
%     if stim_resp==1
%         save(['st_aligned_partialRDM_New_summarized_for_Masoud_region_',num2str(region),'_coherence_',num2str(Coherences(coherence)),'.mat'],'ParCorrelations_Fam_Unfam','ParCorrelations_Fam_Levels','significance_Fam_Unfam','significance_Fam_Levels','ParCorrelations_Fam_Unfam_NP','ParCorrelations_Fam_Levels_NP','significance_Fam_Unfam_NP','significance_Fam_Levels_NP');
%     else
%         save(['rp_aligned_partialRDM_New_summarized_for_Masoud_region_',num2str(region),'_coherence_',num2str(Coherences(coherence)),'.mat'],'ParCorrelations_Fam_Unfam','ParCorrelations_Fam_Levels','significance_Fam_Unfam','significance_Fam_Levels','ParCorrelations_Fam_Unfam_NP','ParCorrelations_Fam_Levels_NP','significance_Fam_Unfam_NP','significance_Fam_Levels_NP');
%     end
end
