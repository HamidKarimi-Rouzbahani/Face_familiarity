...
for region=[1 2]
    for stim_resp=[1 2]
        ...
        firingRatesPooled=nan*ones(1,16,2,2000,60);
        firingRatesAveragePooled=nan*ones(1,16,2,2000);
        trialNumPooled=zeros(1,16,2);
        
        for subj=[1:16 20 21]
            if region==3
                firingRates=nan*ones(62,4,2,2000,60);
                trialNum=zeros(62,16,2);
            else
                firingRates=nan*ones(14,4,2,2000,60);
                trialNum=zeros(14,16,2);
            end
            for correct_incorrect_trials=[1:2]
                ...
                cc=0;
                for counte1=[1 2 4 3]
                    for counte2=[0.22 0.3 0.45 0.55]
                        tmpt=[];
                        cc=cc+1;
                        if subj==13
                            sessions=1;
                        ...
                        for session=sessions
                            indx{cc,session}=([[Exp_data{1,session}.stim.stimTrain.imageCategory]==counte1 & [Exp_data{1,session}.stim.stimTrain.imageNoise]==counte2]);
...
%                 end
 cccc
        ...
        toc
        [region stim_resp]
        save(['DPCA_AllStimuliConds_region_',num2str(region),'_stim_resp_',num2str(stim_resp),'.mat'],'W','V');
    end
end
