...
sessions=[1 2];
iterations=10000;
Correlations_Fam_Unfam=nan*ones(21,71);
Correlations_Fam_Levels=nan*ones(21,71);
Correlations_Fam_Unfam_random=nan*ones(21,71,iterations);
Correlations_Fam_Levels_random=nan*ones(21,71,iterations);
test_on_errors=0;
    ...
    for stim_resp=[1 2]
        for coherence=1:length(Coherences)
            for subj=[1:16 20 21]
                ...
                signals{1,3}=[];
                signals{1,4}=[];
                clearvars -except iterations Correlations_Fam_Unfam_random Correlations_Fam_Levels_random Correlations_Fam_Levels Correlations_Fam_Unfam Coherences coherence region test_on_errors sizes_signals accuracy Exp_data baseline_correction EEG_signals subject sessions steps baseline_span signals2 signals subj cats stim_resp only_answered_trials only_correct_trials only_incorrect_trials only_answered_trials2 only_correct_trials2 only_incorrect_trials2
                ...
                for tws=spans
                    ...
                    for it=1:iterations
                        ...
                        Correlations_Fam_Unfam_random(subj,time,it)=corr(reshape(Familiar_Unfamiliar_Model_RDM_tmp,[size(signals{1,1},3)*size(signals{1,1},3) 1]),reshape(Neural_RDM,[size(signals{1,1},3)*size(signals{1,1},3) 1]),'rows','complete');
                        Correlations_Fam_Levels_random(subj,time,it)=corr(reshape(Familiarity_levels_Model_RDM_tmp,[size(signals{1,1},3)*size(signals{1,1},3) 1]),reshape(Neural_RDM,[size(signals{1,1},3)*size(signals{1,1},3) 1]),'rows','complete');
                    end
                    [region stim_resp coherence subj time]
                end
                
                if stim_resp==1
                    save(['st_aligned_Cor_trls_RDM_New_randoming_correlations_baselined_windowed_region_',num2str(region),'_coherence_',num2str(Coherences(coherence)),'.mat'],'Correlations_Fam_Unfam','Correlations_Fam_Levels','Correlations_Fam_Unfam_random','Correlations_Fam_Levels_random');
                else
                    save(['rp_aligned_Cor_trls_RDM_New_randoming_correlations_baselined_windowed_region_',num2str(region),'_coherence_',num2str(Coherences(coherence)),'.mat'],'Correlations_Fam_Unfam','Correlations_Fam_Levels','Correlations_Fam_Unfam_random','Correlations_Fam_Levels_random');
                end
            end
        end
    end
...
for coherence=[1:4]
    if stim_resp==1
        load(['st_aligned_Cor_trls_RDM_New_randoming_correlations_baselined_windowed_region_',num2str(region),'_coherence_',num2str(Coherences(coherence)),'.mat'],'Correlations_Fam_Unfam','Correlations_Fam_Levels','Correlations_Fam_Unfam_random','Correlations_Fam_Levels_random');
   else
        load(['rp_aligned_Cor_trls_RDM_New_randoming_correlations_baselined_windowed_region_',num2str(region),'_coherence_',num2str(Coherences(coherence)),'.mat'],'Correlations_Fam_Unfam','Correlations_Fam_Levels','Correlations_Fam_Unfam_random','Correlations_Fam_Levels_random');
   end
       plot(smooth(nanmean(Correlations_Fam_Unfam),3));
       hold on;  
end
% figure;
% for coherence=[1:4]
...
%         load(['rp_aligned_Cor_trls_RDM_New_randoming_correlations_baselined_windowed_region_',num2str(region),'_coherence_',num2str(Coherences(coherence)),'.mat'],'Correlations_Fam_Unfam','Correlations_Fam_Levels','Correlations_Fam_Unfam_random','Correlations_Fam_Levels_random');
%    end
%        plot(smooth(nanmean(Correlations_Fam_Levels),3));
%        hold on;  
% end