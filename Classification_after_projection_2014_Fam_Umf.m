...
only_incorrect_trials=0; % plus correct=0 you should also change the loading file names
...
for subj=subjects
    ...
    %% IRLS
    if stim_resp==1
        load(['st_al_weights_fam_unf_subj_',num2str(subj),'conc_Alltrials.mat'],'wp','span','landa','b');
    else
        load(['rp_al_weights_fam_unf_subj_',num2str(subj),'conc_Alltrials.mat'],'wp','span','landa','b');
    end
...
%             indx_max_AUC(subs)=indx_max_AUC(subs)+500./steps;
    ...
    for t=tws
        c=c+1;
        X(:,:,c)=[mean(signals{1,cond1}(:,t,:),3) mean(signals{1,cond2}(:,t,:),3);1 1];
...
legend ('Unfamiliar','Familiar','Location','northwest');
xlabel('Time [ms]')
ylabel('Amplitude [uv]')
hold on;
line([0 0],[-4 6])
