...
for subj=subjects
    ...
    if famous_effect==0
        if only_correct_trials==1
            if stim_resp==1
                if cats==0
                    load(['st_al_weights_coh_level_1_4_subj_',num2str(subj),'conc_incor.mat'],'wp','span','landa','b');
                else
                    load(['st_al_weights_cat_level_1_4_subj_',num2str(subj),'conc_incor.mat'],'wp','span','landa','b');
                end
            else
                if cats==0
                    load(['rp_al_weights_coh_level_1_4_subj_',num2str(subj),'conc_incor.mat'],'wp','span','landa','b');
                else
                    load(['rp_al_weights_cat_level_1_4_subj_',num2str(subj),'conc_incor.mat'],'wp','span','landa','b');
                end
            end
        else
            if stim_resp==1
