...
for stim_resp=[1 2]
    for subj=[2 10 11]
        ...
        if test_on_errors==1
            ...
            %% SVM
            if stim_resp==1
                load(['st_Decoding_fam_unfam_tr_cor_ts_incor_baselined_windowed2.mat'],'accuracy');
            else
                load(['rp_Decoding_fam_unfam_tr_cor_ts_incor_baselined_windowed2.mat'],'accuracy');
            end
            cond1=1;
            cond2=2;
            time=0;
            for tws=1:steps:2000-steps
                ...
                accuracy(subj,1,time)=sum(label==y2)./length(label);
                
                % Unfamiliar and Familiar separately
                accuracy(subj,2,time)=sum(label(1:size(squeeze(signals2{1,cond1}(:,tws,:))',1))==y2(1:size(squeeze(signals2{1,cond1}(:,tws,:))',1)))./size(squeeze(signals2{1,cond1}(:,tws,:))',1);
                accuracy(subj,3,time)=sum(label(size(squeeze(signals2{1,cond1}(:,tws,:))',1)+1:end)==y2(size(squeeze(signals2{1,cond1}(:,tws,:))',1)+1:end))./size(squeeze(signals2{1,cond2}(:,tws,:))',1);
                
                % Familiar categories in order: Famous, Familiar, Self
                accuracy(subj,4,time)=sum(label(sizes_signals(1)+1:sizes_signals(1)+sizes_signals(2))==y2(sizes_signals(1)+1:sizes_signals(1)+sizes_signals(2)))./sizes_signals(2);
                accuracy(subj,5,time)=sum(label(sizes_signals(1)+sizes_signals(2)+1:sizes_signals(1)+sizes_signals(2)+sizes_signals(3))==y2(sizes_signals(1)+sizes_signals(2)+1:sizes_signals(1)+sizes_signals(2)+sizes_signals(3)))./sizes_signals(3);
                accuracy(subj,6,time)=sum(label(sizes_signals(1)+sizes_signals(2)+sizes_signals(3)+1:sizes_signals(1)+sizes_signals(2)+sizes_signals(3)+sizes_signals(4))==y2(sizes_signals(1)+sizes_signals(2)+sizes_signals(3)+1:sizes_signals(1)+sizes_signals(2)+sizes_signals(3)+sizes_signals(4)))./sizes_signals(4);
                
                [test_on_errors stim_resp subj time]
            end
            if stim_resp==1
            ...
            if stim_resp==1
                load(['st_Decoding_fam_unfam_tr_cor_ts_cor_baselined_windowed2.mat'],'accuracy');
            else
                load(['rp_Decoding_fam_unfam_tr_cor_ts_cor_baselined_windowed2.mat'],'accuracy');
            end
            %% SVM
            cond1=1;
            cond2=2;
            ...
            for tws=1:steps:2000-steps
                ...
                for iter=1:10
                    ...
                    [test_on_errors stim_resp subj time iter]
                end
...
if baseline_correction==0
    ...
    if windowed==0
        ...
        if cor_cor==1
            if stim_resp==1
                load(['st_Decoding_fam_unfam_tr_cor_ts_cor_baselined_windowed2.mat']);
            else
