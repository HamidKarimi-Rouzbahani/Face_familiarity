...
for region=[1]
    for stim_resp=[1 2]
        for subj=[1:16 20 21]
            ...
            %             if baseline_correction
            %                 tt=nanmean(nanmean(cat(3,signals{1,1}(:,baseline_span,:),signals{1,2}(:,baseline_span,:)),2),3);
            %                 for i=1:2
            %                     signals{1,i}=signals{1,i}-repmat(tt,[1 2000 size(signals{1,i},3)]);
            %                 end
            %             end
%             if baseline_correction
%                 tt=nanmean([nanmean(nanmean(signals{1,1}(:,baseline_span,:),3),2) nanmean(nanmean(signals{1,2}(:,baseline_span,:),3),2)],2);
%                 for i=1:2
%                     signals{1,i}=signals{1,i}-repmat(tt,[1 2000 size(signals{1,i},3)]);
%                 end
%             end
            ...
            if test_on_errors==1
                ...
                %                 if baseline_correction
                %                     tt=nanmean(nanmean(cat(3,signals2{1,1}(:,baseline_span,:),signals2{1,2}(:,baseline_span,:)),2),3);
                %                     for i=1:2
                %                         signals2{1,i}=signals2{1,i}-repmat(tt,[1 2000 size(signals2{1,i},3)]);
                %                     end
                %                 end
%                 if baseline_correction
%                     tt=nanmean([nanmean(nanmean(signals2{1,1}(:,baseline_span,:),3),2) nanmean(nanmean(signals2{1,2}(:,baseline_span,:),3),2)],2);
%                     for i=1:2
%                         signals2{1,i}=signals2{1,i}-repmat(tt,[1 2000 size(signals2{1,i},3)]);
%                     end
%                 end
                clearvars -except spans steps_average region test_on_errors sizes_signals accuracy subject sessions steps baseline_span baseline_correction signals signals2 subj cats stim_resp only_answered_trials only_correct_trials only_incorrect_trials only_answered_trials2 only_correct_trials2 only_incorrect_trials2
                %% SVM
                cond1=1;
                cond2=2;
                time=0;
                ...
                if stim_resp==1
                    save(['st_Decoding_fam_unfam_tr_cor_ts_incor_baselined_windowed_region_',num2str(region),'_New3.mat'],'accuracy');
                else
                    save(['rp_Decoding_fam_unfam_tr_cor_ts_incor_baselined_windowed_region_',num2str(region),'_New3.mat'],'accuracy');
                end
            end
        end
    end
