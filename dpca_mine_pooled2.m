...
for region=[2]
    for stim_resp=[2]
        ...
        firingRatesPooled=nan*ones(1,4,2,2000,120);
        firingRatesAveragePooled=nan*ones(1,4,2,2000);
        trialNumPooled=zeros(1,4,2);
        
        for subj=[1:16 20 21]
            if region==3
                firingRates=nan*ones(62,4,2,2000,120);
                trialNum=zeros(62,4,2);
            else
                firingRates=nan*ones(14,4,2,2000,120);
...
%         toc
%         [region stim_resp]
%         save(['DPCA_Noise_region_',num2str(region),'_stim_resp_',num2str(stim_resp),'.mat'],'W','V','whichMarg');
        
        %% plotting
        load(['DPCA_Noise_region_',num2str(region),'_stim_resp_',num2str(stim_resp),'.mat']);
