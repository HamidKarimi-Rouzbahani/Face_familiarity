...
levels=0; % for each level of coherence=1; pooled across coherence levels= 0;
region=3;
stim_resp=1;
...
elseif trials==3
    if CatCoh==1
        if stim_resp==1
            load(['st_Decoding_fam_unfam_tr_cor_ts_incor_baselined_windowed_region_',num2str(region),'_v5.mat']);
            accuracy_coherences_st=accuracy;
        else
            load(['rp_Decoding_fam_unfam_tr_cor_ts_incor_baselined_windowed_region_',num2str(region),'_v5.mat']);
            accuracy_coherences_rp=accuracy;
        end
    else
        if stim_resp==1
...
for decoding_type=1:6
    for time=1:199
        if stim_resp==1
            p(decoding_type,time)=signrank(zeros(sum(~isnan(accuracy_coherences_st(:,decoding_type,time))),1)+0.5,accuracy_coherences_st(~isnan(accuracy_coherences_st(:,decoding_type,time)),decoding_type,time)','tail','right');
        else
            p(decoding_type,time)=signrank(zeros(sum(~isnan(accuracy_coherences_rp(:,decoding_type,time))),1)+0.5,accuracy_coherences_rp(~isnan(accuracy_coherences_rp(:,decoding_type,time)),decoding_type,time)','tail','right');
        end
...
if stim_resp==1
    Corrected_p_values_st=ones(6,199);
    for decoding_type=1:6
        Corrected_p_values_st(decoding_type,40:110)=(squeeze(p(decoding_type,40:110)));
        %         Corrected_p_values(coherence,decoding_type,40:110)=mafdr(squeeze(p(coherence,decoding_type,40:110)));
    end
    if region==1
        save('occipito_temporal_decoding_stim_aligned_trained_cor_tested_incor.mat','accuracy_coherences_st','Corrected_p_values_st')
    elseif region==2
        save('fronto_parietal_decoding_stim_aligned_trained_cor_tested_incor.mat','accuracy_coherences_st','Corrected_p_values_st')
    elseif region==3
        save('whole_brain_decoding_stim_aligned_trained_cor_tested_incor.mat','accuracy_coherences_st','Corrected_p_values_st')
    end
    plot(squeeze(nanmean(accuracy_coherences_st(:,1,40:110),1))')
    figure;
    plot(squeeze(nanmean(Corrected_p_values_st(1,40:110),1))')
else
    Corrected_p_values_rp=ones(6,199);
    for decoding_type=1:6
        Corrected_p_values_rp(decoding_type,91:161)=(squeeze(p(decoding_type,91:161)));
        %         Corrected_p_values(coherence,decoding_type,40:110)=mafdr(squeeze(p(coherence,decoding_type,40:110)));
    end
    if region==1
        save('occipito_temporal_decoding_resp_aligned_trained_cor_tested_incor.mat','accuracy_coherences_rp','Corrected_p_values_rp')
    elseif region==2
        save('fronto_parietal_decoding_resp_aligned_trained_cor_tested_incor.mat','accuracy_coherences_rp','Corrected_p_values_rp')
    elseif region==3
        save('whole_brain_decoding_resp_aligned_trained_cor_tested_incor.mat','accuracy_coherences_rp','Corrected_p_values_rp')
    end
    plot(squeeze(nanmean(accuracy_coherences_rp(:,1,91:161),1))')
    figure;
    plot(squeeze(nanmean(Corrected_p_values_rp(1,91:161),1))')
end
