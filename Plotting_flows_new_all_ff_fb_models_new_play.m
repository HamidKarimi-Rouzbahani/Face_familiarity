...
%             load(['st_al_pCor_IMG_occip_front_and_Flow_Novel_SP_pasttime_',num2str(past_time),'_pastwindow_',num2str(past_window),'_coherence_',num2str(Coherences(coherence)),'.mat']);
        ...
        if model==1
            data=(ParCorrelations_Unfamiliar_frnt-ParCorrelations_FF_Unfamiliar)-(ParCorrelations_Unfamiliar_ocpt-ParCorrelations_FB_Unfamiliar);
            plot(spans,nanmean(data),'color',colors(coherence,:),'linewidth',3)
        elseif model==2
            data=(ParCorrelations_Familiar_frnt-ParCorrelations_FF_Familiar)-(ParCorrelations_Familiar_ocpt-ParCorrelations_FB_Familiar);
            plot(spans,nanmean(data),'color',colors(coherence,:),'linewidth',3)
        elseif model==3
            data=(ParCorrelations_Lev_familiar_frnt-ParCorrelations_FF_Lev_familiar)-(ParCorrelations_Lev_familiar_ocpt-ParCorrelations_FB_Lev_familiar);
            plot(spans,nanmean(data),'color',colors(coherence,:),'linewidth',3)
        elseif model==4
            data=(ParCorrelations_Famous_frnt-ParCorrelations_FF_Famous)-(ParCorrelations_Famous_ocpt-ParCorrelations_FB_Famous);
            plot(spans,nanmean(data),'color',colors(coherence,:),'linewidth',3)
        elseif model==5
            data=(ParCorrelations_Personally_frnt-ParCorrelations_FF_Personally)-(ParCorrelations_Personally_ocpt-ParCorrelations_FB_Personally);
            plot(spans,nanmean(data),'color',colors(coherence,:),'linewidth',3)
        elseif model==6
            data=(ParCorrelations_Self_frnt-ParCorrelations_FF_Self)-(ParCorrelations_Self_ocpt-ParCorrelations_FB_Self);
            plot(spans,nanmean(data),'color',colors(coherence,:),'linewidth',3)
        end
        hold on;
        FF_FB_models(:,:,model,coherence)=data;
    end
    if model==1
        legend coh=0.22 coh=0.35 coh=0.45 coh=0.55
        xlabel('time (ms)')
...
% if model==1
%     plots(model)=plot(spans,nanmean(ParCorrelations_Unfamiliar_frnt),'linewidth',3)
% elseif model==2
%     plots(model)=plot(spans,nanmean(ParCorrelations_Famous_frnt),'linewidth',3)
% elseif model==3
%     plots(model)=plot(spans,nanmean(ParCorrelations_Personally_frnt),'linewidth',3)
% elseif model==4
%     plots(model)=plot(spans,nanmean(ParCorrelations_Self_frnt),'linewidth',3)
% end

% if model==1
%     plots(model)=plot(spans,nanmean(ParCorrelations_Unfamiliar_ocpt),'linewidth',3)
% elseif model==2
%     plots(model)=plot(spans,nanmean(ParCorrelations_Famous_ocpt),'linewidth',3)
% elseif model==3
%     plots(model)=plot(spans,nanmean(ParCorrelations_Personally_ocpt),'linewidth',3)
% elseif model==4
%     plots(model)=plot(spans,nanmean(ParCorrelations_Self_ocpt),'linewidth',3)
% end

hold on;
%     if model==4
%         legend([plots(1),plots(2),plots(3),plots(4)],{'unfamiliar','famous','personally','self'});
%         xlabel('time (ms)')
%         ylabel('feedforward/feedback flow')
%     end
%     title(titles{model})
    if model==6
        legend([plots(1),plots(2),plots(3),plots(4),plots(5),plots(6)],{'unfamiliar','familiar','level','famous','personally','self'});
        xlabel('time (ms)')
        ylabel('feedforward/feedback flow')
    end
    line([spans(1) spans(end)],[0 0],'linestyle','-.','color','k')
